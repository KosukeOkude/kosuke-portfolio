interface WorksHintArchiveProps {
  orientation: string;
}

export default function WorksArchiveHint({
  orientation,
}: WorksHintArchiveProps) {
  return (
    <div
    ...
    >
      <span
        className="works-hint-line-track h-px w-full max-w-[4.5rem] shrink-0 rounded-full bg-gradient-to-r from-white/90 via-white/45 to-transparent opacity-95 group-hover:via-white/55 motion-reduce:transition-none md:max-w-[5rem]"
        aria-hidden="true"
      >
        <span className="works-hint-line-shimmer" />
      </span>
